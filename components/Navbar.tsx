      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
                      ...
                      <div className="absolute top-full left-0 w-64 bg-white py-2 z-50 animate-fadeIn">
                        {destinationLinks.map((dest) => (
                          <Link
                            key={dest.name}
                            href={dest.path}
                            className="block px-4 py-2.5 text-[13px] font-semibold text-slate-800 hover:bg-sky-50 hover:text-sky-600 border-b border-slate-100 last:border-0"
          ...
          <div className="hidden md:flex items-center gap-2 sm:gap-3">
            <Link
              href="/custom-plan"
              className="px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-medium text-[13px] uppercase tracking-wider transition-colors flex items-center gap-1.5"
            >
        ...
        <div className="xl:hidden bg-white border-t border-slate-200 px-4 pt-2 pb-6 space-y-2 animate-fadeIn max-h-[85vh] overflow-y-auto">
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.path}
              className={`block px-3 py-2.5 text-[13px] font-bold uppercase tracking-wider border-b border-slate-100 ${isCurrent(item.path)
                  ? 'text-sky-600 bg-sky-50'
          ...
          <div className="pt-4 space-y-2">
            <Link
              href="/custom-plan"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full py-3 bg-sky-600 text-white font-medium text-[13px] uppercase tracking-wider flex items-center justify-center gap-2"
            >
