        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {TREK_STYLES.map((style, idx) => (
            <div
              ...
              className={`p-4 flex flex-col items-center text-center cursor-pointer border transition-all ${
                idx === 0
                  ? 'bg-sky-600 text-white border-sky-600 hover:bg-sky-700'
                  : 'bg-slate-50 text-slate-800 border-slate-200 hover:border-sky-500 hover:bg-sky-50'
              }`}
            >
              <div className={`mb-3 p-3 rounded-none ${idx === 0 ? 'text-sky-100 bg-sky-700/50' : 'text-sky-600 bg-white border border-slate-200'}`}>
