      ...
      <div className="relative z-10 w-full mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 lg:py-24">
        <div className="max-w-4xl">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-600/20 text-sky-200  text-xs font-bold uppercase tracking-wider rounded-sm">
          ...
          <p className="text-slate-200 text-sm sm:text-base lg:text-lg mb-6 leading-relaxed max-w-2xl">
            ...
            <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              <div className="relative flex items-center border-2 border-slate-200 px-3 py-2.5 bg-white focus-within:border-sky-500 transition-colors">
                ...
                <select
                  ...
                  className="w-full bg-transparent text-sm text-slate-800 focus:outline-none cursor-pointer appearance-none"
              ...
              <div>
                <button
                  type="submit"
                  className="w-full h-full min-h-[48px] bg-sky-600 hover:bg-sky-700 text-white font-bold text-sm uppercase tracking-wider transition-colors flex items-center justify-center gap-2 cursor-pointer rounded-sm"
                >
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6">
            <span className="text-xs font-bold text-slate-300 uppercase tracking-wider mr-1">Popular Routes:</span>
            {quickTags.map((tag) => (
              <button
                key={tag}
                onClick={() => onTagClick(tag)}
                className="px-3 py-1.5 bg-slate-800/70 hover:bg-sky-600 hover:text-white border border-slate-700 hover:border-sky-500 text-slate-200 text-xs font-medium transition-all duration-200 rounded-sm"
              >
                {tag}
              </button>
            ))}
